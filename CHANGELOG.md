...
### 글 품질 개선
- **3,000자 분량 기준 추가** — 분석가 단계에서 섹션별 "다룰 내용" 2~3개 항목 명시 의무화
  - 분량 책임을 작가가 아닌 분석가의 섹션 설계 단계로 이관
- **패딩 방지 규칙 추가** — 앞 내용 재요약, "이처럼" 연결어 채우기, 당연한 사실 장황화, 동어반복 명시적 금지
  - 분량 부족 시 덜 다뤄진 항목을 더 깊게 파고드는 방식으로 대체
- **편집장 3,000자 미달 검증** — 미달 시 분석가의 "다룰 내용" 항목 기준으로 보완 지시
