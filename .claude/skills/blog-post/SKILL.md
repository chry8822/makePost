...
## 글 구조
전체 공백 제외 3,000자 이상이 자연스럽게 나올 만큼 섹션을 설계한다.
섹션 수를 늘리는 방식이 아니라 각 섹션에 다룰 내용을 충분히 구체화하는 방식으로.
각 섹션 아래 "다룰 내용"을 2~3개 항목으로 명시해야 한다.
...
- **용어 설명 규칙**: 독자가 처음 접할 수 있는 비일상적 개념·용어가 본문에 처음 등장할 때 1~2문장으로 짧게 설명한다.
  - 대상: 전문 용어(예: SSL 종단, 리버스 프록시, Serverless Function), 약어(CSP, HSTS 등), 배경 지식 없이는 맥락 파악이 어려운 개념
  - 제외: 해당 글의 타겟 독자가 이미 알 것으로 전제되는 내용 (분석가의 타겟 독자 정의를 기준으로 판단)
  - 방식: 별도 섹션이나 박스보다 **본문 흐름 안에** 자연스럽게 녹인다. "~란 ~이다." 형태의 한 문장으로 처리하고 길게 설명하지 않는다.
- **분량 기준**: 분석가가 설계한 "다룰 내용" 항목을 빠짐없이 다루면 3,000자는 자연히 나온다. 분량을 채우기 위해 쓰는 것 금지:
  - 앞에서 한 말 다시 요약
  - "이처럼", "위에서 살펴봤듯이" 류의 연결어로 단락 채우기
  - 독자가 이미 알 만한 당연한 사실 길게 설명
  - 같은 포인트를 다른 표현으로 반복
  → 분량이 모자라면 분석가의 "다룰 내용" 항목 중 덜 다뤄진 것을 더 깊게 파고든다
...
- **콘텐츠 품질 점검:**
  ...
  - 공백 제외 **3,000자 미만**인가 → analysis.md의 "다룰 내용" 항목 중 충분히 다뤄지지 않은 것을 찾아 그 부분을 더 깊게 파고든다. 새로운 단락을 억지로 추가하거나 앞 내용을 요약 반복하는 방식 금지
- **디자인 점검:**
